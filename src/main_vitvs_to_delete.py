...
# from models.baby_cns.cns.benchmark.pipeline import CorrespondenceBasedPipeline, VisOpt
# from models.baby_.utils.perception import CameraIntrinsic
# from models.cns.benchmark.stop_policy import SSIMStopPolicy
# from models.baby_cns.cns import CnsLib
...
def vitvs():
    ...
    try:
        ...
        logging.info(f" Saving Keypoints into {data.result_path}")
        ...
        if data.still_frame_number >= -1:
            ...
            gf = get_frame_at(data.goal_path, data.gf_position)
            gt = True
        else:
            gt, gf = gcap.read()
        ...
        results = []
        never_got = True  # flag to check if we ever got a frame from the goal video
        ...
        while gt and it:
            ...
            data.progress += 1
            data.inf_position += 1
            kp_out_file_name = os.path.join(data.result_path, f"{data.progress}.png")
            ...
            if data.still_frame_number != -1 and never_got:
                never_got = False
...
if __name__ == "__main__":
    ...
    data.still_frame_number = args.reference_still_frame
