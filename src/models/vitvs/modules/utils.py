...
def render_correspondence(
    image1, image2, points1, points2, save_path=None, show_plot=None
):
    ...
    if show_plot is None:
        show_plot = (
            ...
            # and matplotlib.get_backend() != "Agg"
        )

    # Always save if requested or if running in headless mode
    # NOTE: Doing this on every frame makes processing a lot slower
    if save_path or not show_plot:
        if save_path is None:
            save_path = "results/correspondences.png"
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        data.last_result_path = save_path
        plt.savefig(save_path, dpi=150, bbox_inches="tight")
        logging.info(f"Correspondence image saved to {save_path}")
