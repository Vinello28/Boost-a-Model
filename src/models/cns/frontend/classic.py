...
class Classic(FrontendBase):
    ...
    def _get_detector(self, detector: str = "SIFT"):
        create_func_name = detector.strip().upper() + "_create"
        if not hasattr(cv2, create_func_name):
            print(f"[INFO] Cannot load function {create_func_name} from module cv2")
            print("[INFO] Fallback to SIFT_create")
            self.detecor_name = "SIFT"
            create_func_name = "SIFT_create"
        create_func = getattr(cv2, create_func_name)
        return create_func()
    ...
    def _feature_match(self, cur_pos, cur_des):
        indices, dists = self.kdtree.knnSearch(cur_des, knn=2, params=dict(checks=50))
        ...
        if self.ransac and len(matches) > 4:
            ...
            if len(matches) == 4:
                ...
                _, mask = cv2.findEssentialMat(
                    tar_pts, cur_pts, self.intrinsic.K, cv2.RANSAC, 0.999, 3.0
                )
            if mask is not None:
    ...
    def update_target_frame(self, image, mask=None):
        ...
        for k in self.rot90s:
            if k == 0:
                image_rot = image
                mask_rot = mask
                inv_rot_func = lambda x: x
            else:
                image_rot, _, inv_rot_func = rot90(image, k=k)
                if mask is not None:
                    mask_rot, _, _ = rot90(mask, k=k)
                else:
                    mask_rot = None

            tar_kp_r, tar_des_r, success = self._extract_kp_des(image_rot, mask_rot)
            success_any = success_any or success

            if success:
                tar_pos_list.append(
                    inv_rot_func(np.float32([kp.pt for kp in tar_kp_r]))
                )
                tar_des_list.append(tar_des_r)

        if success_any:
            self.tar_img = image
            self.tar_img_changed = True
            self.tar_des_rot = np.concatenate(tar_des_list)
            self.tar_pos = np.concatenate(tar_pos_list)
            self._build_tree()
    ...
    def process_current_frame(self, image):
        ...
        if len(matches) < 3:
            print(f"[INFO] Too few matches (={len(matches)})")
            return None
    ...
    while True:
        ...
        if ret:
            ...
            if corr:
                ...
                t2 = time.time()
                print(
                    f"[INFO] Matching time = {t1-t0:.3f}s, vis time = {t2-t1:.3f}s"
                )
