        ...
        if len(valid_corr) == 0:
            ...
            return Correspondence(
                ...
                valid_mask=np.concatenate(
                    [c.valid_mask for c in valid_corr], axis=0
                ),
                cur_pos_aligned=np.concatenate(
                    [c.cur_pos_aligned for c in valid_corr], axis=0
                ),
...
def plot_corr(corr: Correspondence, color=None, show_keypoints=False, margin=10):
    ...
    out_size = (
        (H, W)
        if len(corr.tar_img.shape) == 2
        else (H, W, corr.tar_img.shape[-1])
    )
    ...
    color = (
        (np.array(color[:, :3]) * 255).astype(int)[:, ::-1]
    ...
    out = put_text(
        ...
        text=[
            ...
            f"Keypoints: {corr.tar_pos.shape[0]}:{corr.cur_pos.shape[0]}",
            f"Matches: {np.sum(corr.valid_mask)}",
        ],
    )
